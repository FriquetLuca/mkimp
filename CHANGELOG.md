...
- Remove the now unused `@types/katex`.

## [6.0.0] - 2026-03-27

Breaking changes.

- Remove `katex` and `highlight.js` from `dependencies`.
- Add use for `highlight` and `latex` in tokenizer and patch the renderer to skip them if wanted.
- Callback to use custom highlighter and latex transpiler.
- Expose `registerToHLJS` if `highlight.js` is used to render so it can handle language not included with it.

## [5.0.1] - 2026-03-27

- Export `RenderTarget` and `LinkRef` types.
- Add a `generateSections` and `generateTableOfContent` helper function.

## [5.0.0] - 2026-03-24

- Conditional rendering added.
- Add a sup and sub notation.
- Add the small notation.
