...
export interface HorizontalToken {
  type: 'horizontal';
  character: string;
}
...
export interface FootnoteEndToken {
  type: 'footnoteEnd';
}

export interface TableOfContentToken {
  type: 'tableOfContent';
}
...
function isBlockToken(
  ...
  return (
    ...
    (lexer.includeCode !== undefined && isIncludeCode(level, line, lines)) ||
...
export class BlockTokenizer {
      ...
      if (this.line < this.content.length) {
        ...
        if (dashes !== null) {
          ...
          while (line < this.content.length) {
            ...
            if (dashEnd !== null && dashEnd[0].length === rowDashes.length) {
              endIndex = line;
              break;
            }
            line++;
  ...
  async spoiler(): Promise<SpoilerToken | undefined> {
    ...
    if (opening.length > 1 && opening[0] === '!' && opening[1] === '>') {
      const title =
        opening.length > 2 ? opening.slice(2, opening.length).trimStart() : '';
      const startIndex = ++this.line;
    ...
    while (this.line < this.content.length) {
      ...
      if (currentItem) {
        ...
        while (this.line < this.content.length) {
          ...
          if (
            (!isBlock && item.level === this.level && !previousIsEmpty) ||
  ...
  async unorderedList(pskip = false): Promise<ListToken | undefined> {
    ...
    const symbol =
      firstElement.content.length > 0 ? firstElement.content[0] : undefined;
    if (
      symbol === undefined ||
    ...
    while (this.line < this.content.length) {
      ...
      if (currentItem) {
        const checkListItem = checkListExtractor(currentItem.content, symbol);
    ...
    if (tableAlign !== undefined && isMaybeTable(secondElement.content)) {
      ...
      if (tableAlign.length === headerCells.length) {
        ...
        while (this.line < this.content.length) {
          ...
          const row = tableLineToCells(stripTableBorder(currentItem.content));
          // Normalize table
  ...
  async footnoteDef(): Promise<boolean> {
    ...
    if (match) {
      ...
      this.line++;
      const lines = [rebuildLineWhitespace(content, this.lexer.tabulation)];
      let previousIsEmpty = false;
      while (this.line < this.content.length) {
        const item = this.content[this.line];
        const itemContent = item.content.trimEnd();
  ...
  async include(): Promise<IncludeToken | undefined> {
    ...
    if (match) {
      const [, loc1, loc2, fromStr, toStr, hShift] = match;
  ...
  async includeCode(): Promise<CodeBlockToken | undefined> {
    ...
    if (match) {
      const [, includePath, includePath2, lang, fromStr, toStr] = match;
  ...
  reflink(): boolean {
    ...
    if (match) {
      const [, ref, linkA, linkB, titleA, titleB, titleC] = match;
