...
export const htmlPatterns: {
    ...
    {
        open: editRegex(/^<\/?(block_names)(\s|\/?>|$)/i).replace("block_names", HTML_BLOCKS).getRegex(),
        close: /^$/, // blank line
        canInterrupt: true
    },
...
export function parseHtmlInline(src: string, start: number, tokenizer: InlineTokenizer): HTMLToken | null {
