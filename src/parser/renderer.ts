...
const RENDERER_FNS: TokenRendering<string> = {
  ...
  async codeblock(token) {
    ...
    if (token.lang && this.hljs && this.hljs.getLanguage(token.lang)) {
      ...
      return `<pre role="region" aria-label="Code block" class="md-precode"><code aria-label="Code" class="md-code ${lang}">${this.hljs.highlight(token.content, { language: token.lang }).value}</code></pre>`;
    }
  ...
  async cell(token) {
    ...
    if (token.header) {
      const style =
        token.align !== 'default' ? ` style="text-align:${token.align}"` : '';
...
export class Renderer {
  ...
  abbrs: AbbrToken[];
  latex: ((token: TexToken) => Promise<string>) | undefined;
  tokens: MdToken[];
  withSection: boolean;
  renderTarget: RenderTarget;
