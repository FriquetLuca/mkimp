...
const RENDERER_FNS: TokenRendering = {
    ...
    listItem(token) {
        const head = token.task ? `<input class="md-checkbox" type="checkbox" disabled${token.checked === true ? ' checked' : ''}> ` : "";
...
export class Renderer {
    ...
    tokens: MdToken[];
    rendering: Partial<TokenRendering>;
    constructor(root: RootToken, rendering: Partial<TokenRendering> = {}) {
    ...
    renderer(tokens: MdToken[]) {
        ...
        for(const token of tokens) {
            const render = this.rendering[token.type];
            if(render !== undefined) {
                result += render.call(this, token as any);
            } else {
