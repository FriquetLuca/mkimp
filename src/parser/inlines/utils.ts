...
export const ENDING_STARS = new Map<"double"|"single", (s: InlineCharacter) => (i: number, src: string) => boolean>([
    ["double", (s: InlineCharacter) => (i: number, src: string) => src[i] === "\n" || (i + 1 < src.length && src[i] === s && src[i + 1] === s)],
    ["single", (s: InlineCharacter) => (i: number, src: string) => src[i] === "\n" || src[i] === s]
]);
