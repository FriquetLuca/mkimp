...
export interface OverlineToken {
  type: 'overline';
  tokens: MdToken[];
}
      ...
      switch (currentSymbol) {
        ...
        case '{':
          if (
            ...
            if (endMeta) {
              this.#cleanBuffer(result);
              const name = this.content.slice(lastIndex + 1, this.index);
              result.push({
                type: 'metadata',
                name,
          ...
          if (count === 3) {
            ...
            if (count === 0) {
              ...
              if (count === 2) {
                ...
                // Italic
                const lastTokens = this.tokenize(
                  ENDING_STARS.get(INLINE_KIND_CHARS.get('italic')!)!(
                    currentSymbol
                  )
                );
                if (
