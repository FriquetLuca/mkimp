...
export function c3(hljs: any) {
  ...
  const C3_KEYWORDS =
    'alias as assert asm bitstruct break case catch const continue define default defer do else enum extern false fault for foreach foreach_r fn generic tlocal if import macro module nextcase null private return static struct switch true try union var while typedef distinct initialize finalize inline';
  const KEYWORDS = {
    keyword: C3_KEYWORDS,
  };
  const SKEYWORDS = {
  ...
  const TYPES = {
    ...
    variants: [
      ...
      {
        begin:
          '\\banyfault!?|\\bany!?|\\bvoid!?|\\bbool!?|\\bchar!?|\\bdouble!?|\\bfloat16!?|\\bbfloat16!?|\\bfloat128!?|\\bint128!?|\\bint!?|\\bichar!?|\\biptr!?|\\bisz!?|\\blong!?|\\bshort!?|\\buint128!?|\\buint!?|\\bulong!?|\\buptr!?|\\bushort!?|\\busz!?|\\bfloat!?|\\btypeid!?|\\bireg!?|\\bureg!?',
      },
  ...
  const CONSTANTS = {
    className: 'symbol',
    variants: [{ begin: '\\b[A-Z]+[0-9A-Z_]*\\b' }],
  };
  ...
  const FUNCTION_DECLARATION = {
    begin:
      '\\s*fn\\s*' + '(' + FUNCTION_TYPE_RE + '[!\\*&\\s]+)+' + FUNCTION_TITLE,
    returnBegin: true,
    end: /[{;=]/,
    excludeEnd: true,
    keywords: KEYWORDS,
