...
export function hlsl(hljs: HLJSApi) {

  let matrixBases = 
    'bool double float half int uint ' +
    'min16float min10float min16int min12int min16uint matrix';
  ...
  for (let base of matrixBases.split(' ')) {
    for (let suffix of matrixSuffixes) {
      if(base === 'matrix' && !suffix.includes('x')) continue;
  ...
  return {
    ...
    keywords: {
      ...
      type:
        ...
        'TextureCube TextureCubeArray struct typedef sampler2D',
