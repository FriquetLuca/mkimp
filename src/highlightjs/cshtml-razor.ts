...
export function razor(hljs: HLJSApi) {
    ...
    const razor_code_block = {
        ...
        contains: [
            {
                variants: razor_code_block_variants.map(function (v) {
    ...
    ;[razor_block, razor_code_block, razor_try_block].forEach(function (mode) {
        const razorModes = contains.filter(function (c) {
            return c !== mode
        })
        const replacerIndex = mode.contains.indexOf(CONTENT_REPLACER)
        mode.contains.splice.apply(
            mode.contains,
            [replacerIndex, 1].concat(razorModes as any) as any
        )
    })
